...
const { handleCors, hasSupabaseConfig, isEmail, json, normalizeEmail, supabaseFetch } = require("../_utils");
...
async function handler(req, res) {
  ...
  if (req.method !== "POST") {
    return json(res, 405, waitlistError("METHOD_NOT_ALLOWED", "Metodo no permitido."));
  }
  ...
  try {
    ...
    return json(res, 400, waitlistError(code, "Payload invalido."));
  }

  const { lead, error, ignored } = buildWaitlistLead(payload, req);
  if (ignored) {
    return json(res, 200, { ok: true, alreadyExists: false });
  }
  ...
  try {
    ...
    return json(res, 200, { ok: true, alreadyExists: result.alreadyExists });
  } catch (error) {
    ...
    return json(res, 500, waitlistError("SERVER_ERROR", "No hemos podido guardar tu email."));
  }
}
