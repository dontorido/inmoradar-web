...
const SEO_KEYWORD_DIFFICULTIES = ["baja", "media", "alta"];
const SEO_KEYWORD_RISK_LEVELS = ["baja", "media", "alta"];
...
function normalizeLandingPath(value) {
  ...
  const path = withoutHost.split(/[?#]/)[0].trim();
  if (!path) return "";
...
function normalizeBacklogItem(raw = {}, index = 0) {
  ...
  const priority = Math.max(0, Math.min(100, normalizePriority(raw.priority ?? raw.search_priority ?? 50)));
  const suggestedLanding = normalizeLandingPath(raw.suggested_landing) || suggestedLandingFor(raw);
  return {
    ...
    status,
    suggested_landing: suggestedLanding,
...
function validateEnum(errors, field, value, allowed) {
  if (!allowed.includes(value)) errors.push(`${field}_invalid`);
}

function validateSeoKeywordOpportunity(input = {}, options = {}) {
  const partial = options.partial === true;
  const errors = [];
  const row = {};

  if (!partial || input.keyword !== undefined) {
    row.keyword = normalizeText(input.keyword, 180);
    if (!row.keyword || row.keyword.length < 3) errors.push("keyword_required");
  }

  if (!partial || input.intent !== undefined) {
    row.intent = normalizeText(input.intent || "analizar_anuncio_inmobiliario", 80);
    validateEnum(errors, "intent", row.intent, SEO_KEYWORD_INTENTS);
  }

  if (!partial || input.page_type !== undefined) {
    row.page_type = normalizeText(input.page_type || "editorial_guide", 80);
    validateEnum(errors, "page_type", row.page_type, SEO_KEYWORD_PAGE_TYPES);
  }

  if (!partial || input.status !== undefined) {
    row.status = normalizeText(input.status || "idea", 40);
    validateEnum(errors, "status", row.status, SEO_KEYWORD_STATUSES);
  }

  if (!partial || input.priority !== undefined || input.search_priority !== undefined) {
    row.priority = normalizePriority(input.priority ?? input.search_priority ?? 50);
    if (row.priority < 0 || row.priority > 100) errors.push("priority_out_of_range");
  }

  if (!partial || input.manual_difficulty !== undefined) {
    row.manual_difficulty = normalizeText(input.manual_difficulty || "media", 40).toLowerCase();
    validateEnum(errors, "manual_difficulty", row.manual_difficulty, SEO_KEYWORD_DIFFICULTIES);
  }

  if (!partial || input.risk_level !== undefined) {
    row.risk_level = normalizeText(input.risk_level || "media", 40).toLowerCase();
    validateEnum(errors, "risk_level", row.risk_level, SEO_KEYWORD_RISK_LEVELS);
  }

  for (const field of ["city", "province", "recommended_cta", "risk_notes", "notes", "created_by", "updated_by"]) {
    if (!partial || input[field] !== undefined) {
      row[field] = normalizeText(input[field], field === "notes" ? 2000 : 500);
    }
  }

  if (!partial || input.suggested_landing !== undefined || input.keyword !== undefined || input.page_type !== undefined || input.city !== undefined) {
    const fallbackBase = { ...input, ...row };
    row.suggested_landing = normalizeLandingPath(input.suggested_landing) || suggestedLandingFor(fallbackBase);
    if (!row.suggested_landing || !row.suggested_landing.startsWith("/")) errors.push("suggested_landing_invalid");
  }

  if (!partial && !row.recommended_cta) row.recommended_cta = "Analiza pisos antes de contactar";
  if (!partial && row.risk_notes === "") row.risk_notes = "Revisar fuentes, prudencia e independencia antes de generar contenido.";

  return { ok: errors.length === 0, errors, row };
}

function seoKeywordBacklogInsertRow(input = {}, options = {}) {
  const result = validateSeoKeywordOpportunity(input, { partial: false });
  ...
  return {
    ...
    row: {
      ...result.row,
      created_by: normalizeText(input.created_by || input.updated_by || "backoffice", 120),
      updated_by: normalizeText(input.updated_by || input.created_by || "backoffice", 120),
      created_at: now,
      updated_at: now
    }
  };
...
function seoKeywordBacklogPatchRow(input = {}, options = {}) {
  const result = validateSeoKeywordOpportunity(input, { partial: true });
  if (!result.ok) return result;
  const now = options.now || new Date().toISOString();
  const row = { ...result.row, updated_at: now };
  if (!row.updated_by) row.updated_by = normalizeText(input.updated_by || "backoffice", 120);
  return { ok: true, errors: [], row };
}

function validateBriefJson(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return { ok: false, errors: ["brief_json_invalid"], brief: null };
  }
  return { ok: true, errors: [], brief: value };
}
...
module.exports = {
  ...
  SEO_KEYWORD_DIFFICULTIES,
  SEO_KEYWORD_INTENTS,
  SEO_KEYWORD_PAGE_TYPES,
  SEO_KEYWORD_RISK_LEVELS,
