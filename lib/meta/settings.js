...
function metaConfig(env = process.env) {
  return {
    ...
    redirectUri: cleanText(env.META_REDIRECT_URI || `${siteUrl(env)}/admin`, 1000),
    ...
    instagramBusinessAccountId: cleanText(env.META_INSTAGRAM_BUSINESS_ACCOUNT_ID, 120),
    siteUrl: siteUrl(env)
  };
}
...
function summarizeConnection(connection = null, env = process.env) {
  ...
  if (status === "connected" && !facebookPageId) status = "needs_page";
...
function shouldRunAutopublisher({ posts = [], settings = defaultSettings(), connection = null, env = process.env, platform = "facebook", now = new Date() } = {}) {
  ...
  if (summary.missing_scopes.length) return { ok: false, reason: `missing_permissions:${summary.missing_scopes.join(",")}`, missing_scopes: summary.missing_scopes };
  if (!["connected", "needs_permissions"].includes(summary.status) && !summary.automatic_available) return { ok: false, reason: "missing_connection" };
