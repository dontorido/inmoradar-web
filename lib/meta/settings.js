...
function metaConfig(env = process.env) {
  return {
    ...
    instagramAppId: cleanText(env.INSTAGRAM_APP_ID, 300),
    instagramAppSecret: cleanText(env.INSTAGRAM_APP_SECRET, 1000),
    instagramRedirectUri: cleanText(env.INSTAGRAM_REDIRECT_URI, 1000),
    instagramGraphVersion: cleanText(env.INSTAGRAM_GRAPH_VERSION || env.META_GRAPH_VERSION || DEFAULT_GRAPH_VERSION, 20),
    instagramAuthUrl: cleanText(env.INSTAGRAM_AUTH_URL || "https://www.instagram.com", 200).replace(/\/+$/, ""),
    instagramApiUrl: cleanText(env.INSTAGRAM_API_URL || "https://api.instagram.com", 200).replace(/\/+$/, ""),
    instagramGraphUrl: cleanText(env.INSTAGRAM_GRAPH_URL || "https://graph.instagram.com", 200).replace(/\/+$/, ""),
    instagramPublishAccountId: cleanText(env.INSTAGRAM_PUBLISH_ACCOUNT_ID, 120),
...
function metaEnvStatus(env = process.env) {
  ...
  const instagramOAuthConfigured =
    configured(config.instagramAppId) &&
    configured(config.instagramAppSecret) &&
    configured(config.instagramRedirectUri);
  ...
  return {
    ...
    instagram_app_id_configured: configured(config.instagramAppId),
    instagram_app_secret_configured: configured(config.instagramAppSecret),
    instagram_redirect_uri_configured: configured(config.instagramRedirectUri),
    ...
    instagram_graph_version: config.instagramGraphVersion,
    instagram_publish_account_id_configured: configured(config.instagramPublishAccountId),
    ...
    facebook_oauth_configured: facebookOAuthConfigured,
...
function summarizeConnection(connection = null, env = process.env) {
  ...
  const facebookOAuthConfigured =
    ...
    configured(config.appSecret) &&
    configured(config.redirectUri);
  const configuredStatus = row.status || (instagramOAuthConfigured || facebookOAuthConfigured ? "needs_connection" : "disconnected");
...
function shouldRunAutopublisher({ posts = [], settings = defaultSettings(), connection = null, env = process.env, platform = "facebook", now = new Date() } = {}) {
  ...
  const platformOauthConfigured = platformKey === "facebook"
    ? configured(config.appId) && configured(config.appSecret) && configured(config.redirectUri)
    : configured(config.instagramAppId) && configured(config.instagramAppSecret) && configured(config.instagramRedirectUri);
  if (!platformOauthConfigured || !configured(config.encryptionKey)) {
