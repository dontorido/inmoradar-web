...
const { cleanText, metaConfig, normalizeScopes, META_REQUIRED_SCOPES } = require("./settings");
...
async function graphGet(path, params = {}, { accessToken = "", env = process.env, fetchImpl = fetch } = {}) {
  ...
  if (!response.ok) {
    const message = payload?.error?.message || payload?.message || `meta_graph_get_failed_${response.status}`;
