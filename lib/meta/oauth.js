...
function buildInstagramAuthorizationUrl({ state = generateOAuthState(), env = process.env, scopes = META_INSTAGRAM_BUSINESS_SCOPES } = {}) {
  ...
  const next = `/oauth/authorize/third_party/?${nextParams.toString()}`;
  return {
    url: `${config.instagramAuthUrl}/accounts/login/?force_authentication&platform_app_id=${encodeURIComponent(config.instagramAppId)}&next=${encodeURIComponent(next)}`,
    state,
    scopes: normalizedScopes,
    provider: "instagram"
  };
