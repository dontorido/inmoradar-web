...
function parseJsonMaybeText(text) {
...
function sanitizeInstagramTokenDiagnostic(value, { code = "", clientSecret = "" } = {}) {
  let clean = sanitizeSecretText(typeof value === "string" ? value : JSON.stringify(value || {}), 1200);
  for (const secret of [code, clientSecret].filter(Boolean)) {
    clean = clean.replace(new RegExp(escapeRegExp(secret), "g"), "[redacted]");
  }
  return clean;
}

function instagramTokenErrorPayload(payload, fallbackMessage) {
  if (payload && typeof payload === "object" && !Array.isArray(payload)) {
    const nested = payload.error && typeof payload.error === "object" ? payload.error : {};
    return {
      error_type: cleanText(nested.type || payload.error_type || payload.type || "", 200),
      error_message: cleanText(nested.message || payload.error_message || payload.message || fallbackMessage, 800),
      error_code: nested.code ?? payload.error_code ?? payload.code ?? null
    };
  }
  return {
    error_type: "",
    error_message: cleanText(payload || fallbackMessage, 800),
    error_code: null
  };
}
...
async function exchangeInstagramAuthorizationCode({ code, env = process.env, fetchImpl = fetch } = {}) {
  ...
  const text = await response.text();
  const shortLived = parseJsonMaybeText(text);
  if (!response.ok || shortLived.error) {
    const fallbackMessage = `meta_instagram_token_failed_${response.status}`;
    const details = instagramTokenErrorPayload(shortLived, fallbackMessage);
    const safeDetails = {
      status: response.status,
      error_type: sanitizeInstagramTokenDiagnostic(details.error_type, { code, clientSecret: config.instagramAppSecret }),
      error_message: sanitizeInstagramTokenDiagnostic(details.error_message, { code, clientSecret: config.instagramAppSecret }),
      error_code: details.error_code,
      response_body: sanitizeInstagramTokenDiagnostic(shortLived, { code, clientSecret: config.instagramAppSecret }),
      client_id: config.instagramAppId,
      redirect_uri: config.instagramRedirectUri,
      grant_type: grantType,
      instagram_app_secret_present: Boolean(config.instagramAppSecret)
    };
    console.error(`[Meta Instagram Token Exchange] ${JSON.stringify(safeDetails)}`);
    const message = safeDetails.error_message || safeDetails.response_body || fallbackMessage;
