...
function validatePublishInput({ post, posts = [], connection, settings: metaSettings = {}, env = process.env } = {}) {
  ...
  const platformMissingScopes = settings.missingRequiredScopes(summary.scopes, settings.requiredScopesForPlatform(platform));
  ...
  if (platform === "facebook" && !summary.has_page_access_token) throw new Error("meta_facebook_page_access_token_missing");
  if (platformMissingScopes.length) throw new Error(`meta_missing_permissions:${platformMissingScopes.join(",")}`);
