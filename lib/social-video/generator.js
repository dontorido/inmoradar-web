...
const { generateVideoBrief, seriesConfig } = require("./videoStrategyInmoRadar");
...
function buildRealAiVideoPack(project) {
  const scenePrompts = project.scenes.map((scene, index) => ({
    id: scene.id,
    title: scene.headline,
    duration_ms: Math.max(1000, Number(scene.end_ms || 0) - Number(scene.start_ms || 0)),
    prompt: [
      `Escena ${index + 1} de un video vertical 9:16 para InmoRadar.`,
      scene.visual_prompt,
      "Personas reales y fotorealistas de fondo en una casa real: piel, ropa, gestos y movimiento natural.",
      "Estilo documental premium, luz natural, camara realista, profundidad de campo suave, nada de ilustracion ni avatares.",
      "Deja espacio libre arriba derecha y abajo derecha para el branding fijo; si el proveedor permite overlays nativos, mantener logo y web durante toda la escena.",
      `Voz/idea: ${scene.voiceover || scene.body || scene.headline}`
    ].join(" ")
  }));

  const masterPrompt = [
    buildGlobalAiPrompt(project),
    "",
    "REQUISITO CRITICO DE REALISMO",
    "Debe parecer video grabado con personas reales en interiores domesticos espanoles. No usar dibujos, motion graphics como fondo principal, personajes 3D, avatares, manos deformes ni caras irreales.",
    "Las personas son ambiente humano y cotidiano: familia, pareja, amigos o comprador mirando un movil/portatil con un anuncio. No deben mirar siempre a camara ni actuar como anuncio publicitario.",
    "Mantener margen visual para que InmoRadar pueda superponer o reforzar textos, logo arriba derecha y firma Inmoradar.app abajo derecha.",
    "",
    "ESCENAS DETALLADAS",
    scenePrompts.map((scene) => `${scene.id}: ${scene.prompt}`).join("\n")
  ].join("\n");

  return {
    status: "requires_external_ai_video_renderer",
    production_note:
      "El export local de InmoRadar es una maqueta. Para personas reales, genera un clip con este pack en una IA de video y subelo como 'Clip IA/personas reales' antes de exportar.",
    master_prompt: masterPrompt,
    negative_prompt:
      "ilustracion, cartoon, 3d render, avatar, stock corporativo exagerado, manos deformes, caras deformes, ojos raros, texto ilegible, logos externos, watermark, celebridades, personas mirando a camara todo el tiempo, vivienda de lujo irreal, promesas absolutas",
    scene_prompts: scenePrompts,
    music_prompt: project.music_prompt,
    overlay_instructions: {
      logo: "InmoRadar arriba derecha durante todo el video",
      website: `${project.branding.websiteText} abajo derecha durante todo el video`,
      safe_zone_px: project.branding.safeZonePx
    }
  };
}
...
function sceneTimeToMs(time, fallbackIndex, fallbackDurationSeconds, totalScenes) {
  const match = String(time || "").match(/([\d.]+)\s*-\s*([\d.]+)s/i);
  if (match) {
    return {
  ...
  const timing = sceneTiming(totalScenes, fallbackDurationSeconds)[fallbackIndex] || { start_ms: 0, end_ms: fallbackDurationSeconds * 1000 };
  return timing;
}

function buildStrategyScenes(brief, visualStyle) {
  const scenes = Array.isArray(brief.scene_by_scene) ? brief.scene_by_scene : [];
  const durationSeconds = Number(brief.duration || 28);
  const backdrop = VISUAL_BACKDROPS[visualStyle] || VISUAL_BACKDROPS.hogar_cotidiano;
  return scenes.map((scene, index) => {
    const timing = sceneTimeToMs(scene.time, index, durationSeconds, scenes.length || 1);
    const isHook = index === 0;
    const isCta = index === scenes.length - 1;
    return {
      id: `scene-${String(index + 1).padStart(2, "0")}-${isHook ? "hook" : isCta ? "cta" : "analysis"}`,
      ...timing,
      role: isHook ? "hook" : isCta ? "cta" : "analysis",
      headline: scene.overlay || scene.voiceover || brief.hook,
      body: scene.voiceover || "",
      voiceover: scene.voiceover || "",
      on_screen_text: scene.overlay || scene.voiceover || "",
      visual_prompt: [
        scene.visual || "Mockup de analisis InmoRadar con datos inmobiliarios claros.",
        `Fondo humano realista y cotidiano: ${backdrop.prompt}.`,
        "Anuncio anonimizado, sin direccion exacta, telefono, nombre de anunciante ni datos personales.",
        "Textos grandes, numeros destacados, ritmo rapido y zona segura para marca."
      ].join(" "),
      motion: isHook
        ? "corte rapido, zoom al dato principal y entrada inmediata del hook"
        : isCta
          ? "cierre con pregunta grande, marca estable y CTA suave"
          : "cambio visual cada 2-3 segundos con tarjetas de datos y zooms cortos",
      ai_instruction:
        "Crear una pieza que parezca analisis util, curioso o educativo; InmoRadar aparece como herramienta dentro de la historia, no como anuncio directo."
    };
  });
}
...
function generateSocialVideoProject(input = {}) {
  ...
  const shouldUseGrowthStrategy = Boolean(
    input.series_id ||
      input.seriesId ||
      input.platform ||
      input.objective ||
  ...
  const growthBrief = shouldUseGrowthStrategy
    ? generateVideoBrief({
        seriesId: input.series_id || input.seriesId,
        platform: input.platform,
        objective: input.objective,
        duration: durationSeconds,
        tone,
        cta: input.growth_cta || input.cta_text,
        propertyData: input.property_data || input.propertyData || {
          ciudad: city || undefined
        }
      })
    : null;
  const scenes = growthBrief
    ? buildStrategyScenes(growthBrief, visualStyle)
    : buildScenes({ topic, topicConfig, city, durationSeconds, tone, audience, cta, visualStyle });
  const now = new Date().toISOString();
  const id = `sv_${crypto.randomUUID ? crypto.randomUUID() : crypto.randomBytes(10).toString("hex")}`;
  const projectDuration = growthBrief ? growthBrief.duration : durationSeconds;
  const topicLabel = growthBrief ? growthBrief.series : topicConfig.label;
  const project = {
    ok: true,
    id,
    generated_at: now,
    engine: growthBrief ? "growth_video_strategy" : "controlled_ai_storyboard",
    status: "storyboard_ready",
    topic: growthBrief ? growthBrief.series_id : topic,
    topic_label: topicLabel,
    title: growthBrief ? `${growthBrief.title} | InmoRadar` : `${topicConfig.label}${city ? ` en ${city}` : ""} | InmoRadar`,
    city: city || null,
    duration_seconds: projectDuration,
    format: FORMAT,
    tone,
    ...
    cta,
    cta_text: growthBrief ? growthBrief.cta : CTA_COPY[cta] || CTA_COPY.install,
    branding,
    scenes,
    caption: growthBrief ? growthBrief.caption : buildCaption({ city, topicConfig, tone, cta }),
    hashtags: growthBrief ? growthBrief.hashtags : ["#InmoRadar", "#Vivienda", "#ComprarCasa", "#Alquiler", "#Inmobiliario"].slice(0, 5),
    growth_strategy: growthBrief,
    overlays: growthBrief ? growthBrief.overlays : scenes.map((scene) => scene.on_screen_text).filter(Boolean),
    ab_variants: growthBrief ? growthBrief.ab_variants : null,
    disclaimer: growthBrief ? growthBrief.disclaimer : "Estimaciones orientativas. No es una tasacion.",
    music_direction: musicConfig.direction,
    music_prompt: `Crear musica instrumental para video vertical inmobiliario: ${musicConfig.direction}. Duracion ${projectDuration}s. Sin voces, sin samples reconocibles, loop limpio y discreto.`,
    render_contract: {
      preview_html: true,
      storyboard: true,
      thumbnail: true,
      mp4_final: "browser_media_recorder_mock_or_uploaded_clip",
      real_people_final: "external_ai_video_renderer_or_uploaded_clip_required",
      webm_fallback: true,
      audio_track: true,
    ...
    quality_checks: [
      ...
      "real_people_ai_pack_ready",
      "music_track_present",
      "central_content_not_covered",
      "no_absolute_claims",
      "no_fake_price_data",
      ...(growthBrief
        ? Object.entries(growthBrief.quality_check || {})
            .filter(([, value]) => value === true)
            .map(([key]) => `growth_${key}`)
        : [])
    ]
  };
  project.real_ai_video = buildRealAiVideoPack(project);
