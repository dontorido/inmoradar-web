...
const RUNWAY_MIN_DURATION_SECONDS = 2;
const RUNWAY_MAX_DURATION_SECONDS = 10;
const RUNWAY_MAX_PROMPT_TEXT_CHARS = 650;
const RUNWAY_TEXT_ONLY_RATIO = "720:1280";
const RUNWAY_LANDSCAPE_RATIO = "1280:720";
...
function normalizeRunwayRatio(value, options = {}) {
  ...
  if (ratio === "1280:720" || ratio === "16:9" || ratio === "1280:768") return RUNWAY_LANDSCAPE_RATIO;
  if (ratio === "720:1280" || ratio === "9:16" || ratio === "768:1280") return RUNWAY_VERTICAL_IMAGE_RATIO;
...
function normalizeRunwayDuration(value) {
  const duration = Number.parseInt(String(value || RUNWAY_MIN_DURATION_SECONDS), 10);
  if (!Number.isFinite(duration)) return RUNWAY_MIN_DURATION_SECONDS;
  return Math.max(RUNWAY_MIN_DURATION_SECONDS, Math.min(RUNWAY_MAX_DURATION_SECONDS, duration));
}
...
function buildRunwayTextToVideoRequest({ project, model, durationSeconds, ratio, sceneIndex, promptImage }) {
  ...
  const endpoint = hasPromptImage ? "image_to_video" : "text_to_video";
  const request = {
    endpoint,
    model: estimate.model,
    promptText: buildRunwayPrompt(project, sceneIndex),
    ratio: normalizeRunwayRatio(ratio, { textOnly: !hasPromptImage }),
...
function buildRunwayFallbackRequest(request = {}) {
  return {
    ...
    ratio: normalizeRunwayRatio(request.ratio, { textOnly: !request.promptImage }),
...
async function createRunwayTextToVideo({ apiSecret, fetchImpl = fetch, request, timeoutMs = 20000 }) {
  ...
  const endpoint = request.endpoint || (request.promptImage ? "image_to_video" : "text_to_video");
  ...
  if (!response.ok && !request.promptImage && isRunwayBodyValidationError(response.status, payload)) {
    const fallbackEndpoint = "text_to_video";
    const fallbackRequest = buildRunwayFallbackRequest(request);
    const fallbackResponse = await postRunwayRequest({ apiSecret, endpoint: fallbackEndpoint, fetchImpl, request: fallbackRequest, timeoutMs });
    ...
    throw new RunwayApiError(runwayErrorMessage(fallbackPayload, fallbackResponse.status), {
      ...
      payload: {
        ...
        fallback_request_summary: runwayRequestSummary({ endpoint: fallbackEndpoint, ...fallbackRequest })
      }
    });
  }
  if (!response.ok) {
...
module.exports = {
  ...
  RUNWAY_LANDSCAPE_RATIO,
  RUNWAY_MAX_DURATION_SECONDS,
  RUNWAY_MIN_DURATION_SECONDS,
