...
function cleanMetadataText(value, maxLength = 160) {
...
function referrerDomain(value) {
  ...
  try {
    ...
    return cleanMetadataText(url.hostname.replace(/^www\./, ""), 120);
  } catch {
    return null;
  }
}

function normalizeUsageUtm(input = {}) {
  const raw = input.utm && typeof input.utm === "object" ? input.utm : {};
  const utm = {
    source: cleanMetadataText(raw.source || input.utm_source || input.utmSource, 120),
    medium: cleanMetadataText(raw.medium || input.utm_medium || input.utmMedium, 120),
    campaign: cleanMetadataText(raw.campaign || input.utm_campaign || input.utmCampaign || input.campaign, 160)
  };
...
function extensionUsageEventFromInput(input = {}, headers = {}, options = {}) {
  ...
  const utm = normalizeUsageUtm(input);
  ...
  return {
    ...
    metadata: {
      ...
      install_source: cleanMetadataText(
        firstValue(
      ...
      campaign: cleanMetadataText(firstValue(input.campaign, input.utm_campaign, input.utmCampaign, utm.campaign), 160),
      referrer_domain: referrerDomain(input.referrer || input.referrer_url || input.referrerUrl),
