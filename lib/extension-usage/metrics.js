...
function normalizeAcquisitionToken(value, fallback = "unknown", maxLength = 80) {
  const text = String(value || "")
    .trim()
    .toLowerCase()
    .replace(/\s+/g, "-")
...
function acquisitionMetadata(input = {}) {
  ...
  const utm = objectValue(input.utm || metadata.utm);
  return {
    ...
    install_source: normalizeAcquisitionToken(
      metadata.install_source || metadata.installSource || input.install_source || input.installSource || "",
      "",
      80
    ) || null,
    acquisition_source: normalizeAcquisitionToken(
      metadata.acquisition_source || metadata.acquisitionSource || input.acquisition_source || input.acquisitionSource || "",
      "",
      80
    ) || null,
    utm_source: normalizeAcquisitionToken(metadata.utm_source || metadata.utmSource || input.utm_source || input.utmSource || utm.source || "", "", 80) || null,
    utm_medium: normalizeAcquisitionToken(metadata.utm_medium || metadata.utmMedium || input.utm_medium || input.utmMedium || utm.medium || "", "", 80) || null,
    utm_campaign: normalizeAcquisitionToken(
      metadata.utm_campaign || metadata.utmCampaign || input.utm_campaign || input.utmCampaign || utm.campaign || "",
      "",
      120
    ) || null,
    campaign: normalizeAcquisitionToken(metadata.campaign || input.campaign || "", "", 120) || null
  };
}
...
function extensionUsageEventFromInput(input = {}, headers = {}, options = {}) {
  ...
  return {
    ...
    source: "extension",
    metadata: acquisitionMetadata(input)
  };
}
...
function acquisitionIdentityFromRow(row = {}) {
  const metadata = objectValue(row.metadata);
  const utm = objectValue(metadata.utm);
  ...
  const rawSource =
    ...
    metadata.source ||
    (rowSource && rowSource !== "extension" ? row.source : "");
  const rawMedium =
    metadata.utm_medium ||
    metadata.utmMedium ||
    utm.medium ||
    metadata.acquisition_medium ||
    metadata.acquisitionMedium ||
    metadata.medium ||
    "";
  const rawCampaign =
    ...
    utm.campaign ||
    metadata.campaign ||
    metadata.acquisition_campaign ||
    metadata.acquisitionCampaign ||
    "";
  return {
    source: normalizeAcquisitionToken(rawSource, "unknown"),
    medium: normalizeAcquisitionToken(rawMedium, "unknown"),
    campaign: normalizeAcquisitionToken(rawCampaign, "-")
  };
}

function isAttributedIdentity(identity = {}) {
  return identity.source !== "unknown" || identity.medium !== "unknown" || identity.campaign !== "-";
}

function sessionAcquisitionIdentity(rows = []) {
  const identities = rows.map(acquisitionIdentityFromRow);
  return identities.find(isAttributedIdentity) || identities[0] || { source: "unknown", medium: "unknown", campaign: "-" };
}
...
function finalizeUsageSession(session) {
  ...
  const identity = sessionAcquisitionIdentity(rows);
...
function buildUsageSessions(rows = []) {
  const safeRows = Array.isArray(rows) && rows.every((row) => Number.isFinite(row?._created_ms))
    ? rows
    : normalizeUsageRows(rows);
...
function buildUserClassifications(userStats, knownUsersBeforeRange = new Set()) {
  const classifications = new Map();
  for (const [user, stats] of userStats.entries()) {
    const hasRangeRecurrence = stats.days.size > 1 || stats.sessions.size > 1;
    if (!knownUsersBeforeRange.has(user)) {
      classifications.set(user, "new");
    } else if (hasRangeRecurrence) {
      classifications.set(user, "returning");
    } else {
      classifications.set(user, "unclassified");
    }
  }
  return classifications;
}

function classificationCounts(users = [], classifications = new Map()) {
  const counts = {
    new_users: 0,
    returning_users: 0,
  ...
  users.forEach((user) => {
    const classification = classifications.get(user) || "unclassified";
    if (classification === "new") counts.new_users += 1;
    else if (classification === "returning") counts.returning_users += 1;
    else counts.unclassified_users += 1;
  });
  return counts;
}

function acquisitionGroupKey(identity = {}) {
  return `${identity.source || "unknown"}|${identity.medium || "unknown"}|${identity.campaign || "-"}`;
}

function createAcquisitionGroup(identity = {}) {
  return {
    source: identity.source || "unknown",
    medium: identity.medium || "unknown",
    campaign: identity.campaign || "-",
    users: new Set(),
    new_users: 0,
    returning_users: 0,
    unclassified_users: 0,
    sessions: 0,
    cta_installation: null,
    chrome_store_clicks: null,
    activations: 0,
    analyses: 0,
    page_views: null,
    events: 0,
    visit_to_cta_rate: null,
    cta_to_analysis_rate: null
  };
}

function buildAcquisitionGroups(rows, sessions, userStats, classifications) {
  const groups = new Map();
  const sessionIdentityByKey = new Map(sessions.map((session) => [session.key, session]));
  const firstSessionByUser = new Map();

  const ensureGroup = (identity) => {
    const key = acquisitionGroupKey(identity);
    if (!groups.has(key)) groups.set(key, createAcquisitionGroup(identity));
  ...
  sessions.forEach((session) => {
    const identity = {
      ...
      medium: session.medium || "unknown",
      campaign: session.campaign || "-"
    };
    const group = ensureGroup(identity);
    group.sessions += 1;
    if (session.anonymous_id_hash) {
      const current = firstSessionByUser.get(session.anonymous_id_hash);
      if (!current || session.start_ms < current.start_ms) firstSessionByUser.set(session.anonymous_id_hash, session);
    }
  });

  for (const user of userStats.keys()) {
    const session = firstSessionByUser.get(user);
    const identity = session
      ? { source: session.source || "unknown", medium: session.medium || "unknown", campaign: session.campaign || "-" }
      : { source: "unknown", medium: "unknown", campaign: "-" };
    const group = ensureGroup(identity);
    group.users.add(user);
    const classification = classifications.get(user) || "unclassified";
    if (classification === "new") group.new_users += 1;
    else if (classification === "returning") group.returning_users += 1;
    else group.unclassified_users += 1;
    if (userStats.get(user)?.has_completed_analysis) group.activations += 1;
  }

  rows.forEach((row) => {
    const session = row._session_key ? sessionIdentityByKey.get(row._session_key) : null;
    const identity = session
      ? { source: session.source || "unknown", medium: session.medium || "unknown", campaign: session.campaign || "-" }
      : acquisitionIdentityFromRow(row);
    const group = ensureGroup(identity);
    group.events += 1;
    if (COMPLETED_ANALYSIS_EVENTS.has(row.event_name)) group.analyses += 1;
  });
  ...
  return Array.from(groups.values())
    .map((group) => ({
      ...
      activation_rate: group.users.size ? roundRatio((group.activations / group.users.size) * 100) : 0,
      metric_source: "extension_usage"
    }))
    .sort(
      (a, b) =>
        b.users - a.users ||
...
function buildUsageTimeseries(rows, sessions, userStats, options = {}) {
  ...
  const classifications = options.userClassifications || buildUserClassifications(userStats, knownUsersBeforeRange);
  ...
  return dateKeys.map((date) => {
    ...
    const newUsers = Array.from(users).filter(
      (user) => classifications.get(user) === "new" && firstDateByUser.get(user) === date
    ).length;
    const returningUsers = Array.from(users).filter(
      (user) => classifications.get(user) === "returning" || (classifications.get(user) === "new" && firstDateByUser.get(user) !== date)
    ).length;
    const unclassifiedUsers = Math.max(0, users.size - newUsers - returningUsers);
...
function summarizeExtensionUsage(rows = [], now = new Date(), options = {}) {
  ...
  const userClassifications = buildUserClassifications(userStats, knownUsersBeforeRange);
  const userClassificationCounts = classificationCounts(users, userClassifications);
  ...
  const kpis = {
    ...
    new_users: userClassificationCounts.new_users,
    returning_users: userClassificationCounts.returning_users,
    unclassified_users: userClassificationCounts.unclassified_users,
  ...
  const acquisition = buildAcquisitionGroups(safeRows, sessions, userStats, userClassifications);
