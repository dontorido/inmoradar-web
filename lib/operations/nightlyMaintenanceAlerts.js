...
function parseNightlyMaintenanceReport(markdown, metadata = {}) {
  ...
  const structuredRisks = firstOwnValue(structured, ["risk_domains", "riskDomains", "risks"]);
  const status = {
    ...inferred,
    ...structured,
  ...
  status.risks = structuredRisks !== undefined
    ? normalizeStringArray(structuredRisks)
    : normalizeStringArray(status.risks || status.risk_domains || status.riskDomains || inferred.risks);
  return status;
}
