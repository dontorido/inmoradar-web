...
function normalizeStatus(value) {
  ...
  if (/deten|stop|stopped|blocked|bloque/.test(normalized)) return "stopped";
...
function parseNightlyMaintenanceReport(markdown, metadata = {}) {
  ...
  status.sensitive_files = normalizeStringArray(status.sensitive_files || status.sensitiveFiles || inferred.sensitive_files);
...
function buildNightlyMaintenanceAlerts(status = {}, options = {}) {
  const alerts = [];
  const now = options.now || status.generated_at || status.created_at || new Date().toISOString();
  ...
  if (status.tests_passed === false) {
    alerts.push(
      alertBase(
        ...
        "La auditoria nocturna detecto tests fallidos. Revisa el informe antes de continuar con cambios o push.",
        { now }
      )
    );
  }
