...
function cleanPath(value, maxLength = 180) {
...
function sanitizeMetadataValue(value, depth = 0) {
  ...
  if (typeof value === "object") {
    ...
    Object.entries(value).slice(0, MAX_METADATA_KEYS).forEach(([key, nested]) => {
      ...
      const safeValue =
        safeKey === "landing_path"
          ? cleanPath(nested)
          : safeKey === "click_timestamp"
            ? cleanText(nested, 40)
            : sanitizeMetadataValue(nested, depth + 1);
  ...
  const withoutSensitiveFragments = text
    .replace(URL_PATTERN, "[url]")
    .replace(PHONE_PATTERN, "[number]")
    .replace(LONG_NUMBER_PATTERN, "[number]");
  return withoutSensitiveFragments || null;
}
