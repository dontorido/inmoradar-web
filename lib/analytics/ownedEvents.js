...
const DEFAULT_OWNED_ANALYTICS_EVENT_RATE_LIMIT = 120;
const DEFAULT_OWNED_ANALYTICS_EVENT_RATE_LIMIT_WINDOW_MS = 60 * 1000;
...
const REDACTED_METADATA_KEY_FINGERPRINTS = new Set([
  "price",
  "precio",
  "pricem2",
  "priceeurm2",
...
function metadataKeyFingerprint(value) {
  return cleanToken(value, 80).replace(/[^a-z0-9]+/g, "");
}

function isRedactedMetadataKey(value) {
  const safeKey = cleanToken(value, 60);
  const fingerprint = metadataKeyFingerprint(value);
  return (
    !safeKey ||
    REDACTED_METADATA_KEYS.has(safeKey) ||
    REDACTED_METADATA_KEY_FINGERPRINTS.has(fingerprint) ||
    safeKey.includes("email") ||
...
function sanitizeMetadataValue(value, depth = 0) {
  ...
  if (typeof value === "object") {
    ...
    Object.entries(value).slice(0, MAX_METADATA_KEYS).forEach(([key, nested]) => {
      ...
      if (isRedactedMetadataKey(key)) return;
...
function analyticsRateLimitIdentity(req, payload = {}) {
  return [
    String(req?.headers?.["x-forwarded-for"] || req?.headers?.["x-real-ip"] || req?.socket?.remoteAddress || "").slice(0, 80),
    String(req?.headers?.["user-agent"] || "").slice(0, 160),
    cleanToken(payload.anonymous_session_id || payload.anonymousSessionId || "", 120),
    cleanUrl(payload.page_path || payload.pagePath || payload.path || "/", 300) || "/"
  ].join("|");
}
...
async function ownedAnalyticsEventPayload(req, res = null) {
  if (req.method !== "POST") {
  ...
  const rateLimit = await checkDurableRateLimit(req, {
    ...
    identity: analyticsRateLimitIdentity(req, payload),
    maxRequests: process.env.ANALYTICS_EVENT_RATE_LIMIT_MAX || DEFAULT_OWNED_ANALYTICS_EVENT_RATE_LIMIT,
    windowMs: process.env.ANALYTICS_EVENT_RATE_LIMIT_WINDOW_MS || DEFAULT_OWNED_ANALYTICS_EVENT_RATE_LIMIT_WINDOW_MS
  });
...
module.exports = {
  ...
  DEFAULT_OWNED_ANALYTICS_EVENT_RATE_LIMIT,
  DEFAULT_OWNED_ANALYTICS_EVENT_RATE_LIMIT_WINDOW_MS,
  MAX_BODY_BYTES,
  analyticsRateLimitIdentity,
