  ...
  return Array.from(groups.values())
    .map((page) => {
      ...
      const enriched = {
        ...
        calculator_to_install_rate: rate(installs, page.calculator_used),
...
function findHighInteractionLowInstallPages(pages = []) {
  return pages
    ...
    .map((page) => ({
      ...
      city: page.city || "",
      score: page.performance_score,
      reason: `${page.interaction_count} interacciones SEO sin clicks de instalacion`
    }));
}

function findCalculatorInstallPages(pages = []) {
  return pages
    .filter((page) => page.calculator_used_count > 0 && page.install_intent > 0)
    .slice(0, 8)
    .map((page) => ({
      ...
      install_intent: page.install_intent,
...
function findCalculatorLowConversionPages(pages = []) {
  return pages
    .filter((page) => page.calculator_used_count >= 3 && page.calculator_to_install_rate < 5)
    .slice(0, 8)
    .map((page) => ({
      ...
      city: page.city || "",
      score: page.performance_score,
      reason: `${page.calculator_used_count} usos de calculadora y ${page.calculator_to_install_rate}% instalacion`
    }));
}
...
function recommendFutureContent(pages = []) {
  ...
  highInteractionLowInstall.slice(0, 4).forEach((item) => {
    recommendations.push({
      ...
      detail: "Hay interaccion SEO, pero no hay intencion de instalacion. Probar CTA contextual junto al bloque que genera interaccion.",
      source_page: item.page
    });
  });
  ...
  calculatorLowConversion.slice(0, 4).forEach((item) => {
    recommendations.push({
      ...
      detail: "La calculadora se usa, pero no convierte en instalacion. Probar CTA inmediato tras el resultado y ejemplo de siguiente paso.",
      source_page: item.page
    });
  });
